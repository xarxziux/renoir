const {trampoline} = require ('../tmp/core/trampoline.js');
...
module.exports = assert => {
    ...
    assert.equal (typeof trampoline, 'function',
        'The trampoline module returns a function and...');
    
    const addTramp = trampoline (addUp);
    
    assert.equal (typeof addTramp, 'function',
    ...
    assert.equal (addTramp ({current: 1001}), 1001,
        'that function returns the expected result and...');
    assert.equal (addTramp ({current: 1234567}), 1234567,
