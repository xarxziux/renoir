const {typeCheck} = require ('../tmp/core/type_check.js');
const {Left, Right} = require ('../tmp/core/either.js');
...
module.exports = assert => {
    ...
    assert.equal (typeof trueCheck, 'function',
    ...
    assert.equal (stringEth.isEither(), true,
        'that function takes a right either and returns an either' +
                ' and...');
    assert.equal (stringEth.isRight(), true,
        'that either is a right either and...');
    assert.equal (rightEth, stringEth,
        'that either is the same one that was passed in and...');
    
    const failedEth = testString (leftEth);
    
    assert.equal (failedEth.isEither(), true,
        'that function takes a left either and returns an either' +
                ' and...');
    assert.equal (failedEth.isLeft(), true,
        'that either is a left either and...');
    assert.equal (leftEth, failedEth,
        'that either is the same one that was passed in and...');
    ...
    assert.equal (failEth, noCheck (failEth),
    ...
    assert.equal (numFail.left().message,
        'Type check failed: string expected but number detected.');
    assert.equal (arrFail.left().message,
