...
import * as utils from '../src/renoir.js'
...
const testNull = assert => {
  ...
  assert.plan(26)
  assert.equal(typeof isNull, 'function', 'isNull is a function and...')
  assert.equal(
    typeof isNotNull,
    'function',
...
const testError = assert => {
  ...
  assert.plan(14)
  assert.equal(typeof isError, 'function', 'isError is a function and...')
...
export default assert => {
  ...
  assert.test(
    'Test that the isError function works as expected',
    testError
  )

  assert.test(
    'Test that the getBetween function works as expected',
    testGetBetween
  )
}
